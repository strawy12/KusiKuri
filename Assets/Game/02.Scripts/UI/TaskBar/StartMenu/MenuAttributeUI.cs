...
{
    [SerializeField] private float expandDelayTime = 0.8f;
    ...
    private void CheckClose(EventParamData hits)
    {
        if (isStop) { return; }
        if (!isOpen) { return; }
    ...
    public void Close(EventParamData data)
    {
        Close();
    }
    ...
    private void ActivePowerPanel(EventParamData data)
    {
        //TODO: EventParamData
        isStop = data.boolParam;
    ...
    private void ToggleMenu(EventParamData d)
    {
        if (isExpand)
        {
            ContractPanel();
